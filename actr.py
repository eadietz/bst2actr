...
class interface():
    ...
    def run_command(self, command, command_name, model, id, params):
        ...
        if error:
            f['result'] = None
            f['error'] = {
                'message': "Error %s while evaluating a command in Python for command: %s, cognitive-model-specification: %s, parameters: %s" % (
    ...
    def echo_output(self):
        if not (self.echo):
            ...
            self.send("monitor", "cognitive-model-specification-trace", "python-echo" + str(self.echo_count))
    ...
    def no_output(self):
        ...
        if self.echo:
            self.send("remove-monitor", "cognitive-model-specification-trace", "python-echo" + str(self.echo_count))
...
def current_model():
    try:
        ...
        m = current_connection.evaluate_single('current-cognitive-model-specification')
    return m
...
def set_current_model(name):
    if name.lower() in (x.lower() for x in mp_models()):
        ...
        print("%s is not one of the currently available cognitive-cognitive-model-specification-specification: %s" % (name, mp_models()))
...
def load_act_r_model(path):
    return current_connection.evaluate_single("load-act-r-cognitive-model-specification", path)
...
def model_output(output_string):
    return current_connection.evaluate_single("cognitive-model-specification-output", output_string)
...
def mp_models():
    return current_connection.evaluate_single("mp-cognitive-cognitive-model-specification-specification")
